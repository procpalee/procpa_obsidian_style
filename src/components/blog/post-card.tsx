...
export function PostCard({ post }: { post: PostMeta }) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group flex flex-col overflow-hidden rounded-2xl border border-border/60 transition-all hover:-translate-y-0.5 hover:border-foreground/40 hover:shadow-sm"
    >
      {/* 썸네일 */}
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-secondary">
        {post.cover ? (
          // 외부 URL 썸네일 허용 — next/image 도메인 설정 불필요하도록 일반 img 사용
