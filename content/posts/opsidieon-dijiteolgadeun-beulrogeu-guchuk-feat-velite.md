---
title: 옵시디언 디지털가든 블로그 구축(feat. Velite)
description: 옵시디언에서 작성한 마크다운을 그대로 웹에 올리는 개인 지식 플랫폼을 만든 과정을 정리합니다. 제작 동기부터 기술 스택, 주요 기능, 사용 방법까지.
date: 2026-06-28
category: project
slug: opsidieon-dijiteolgadeun-beulrogeu-guchuk-feat-velite
draft: false
---
![image.png](https://cdn.jsdelivr.net/gh/procpalee/my_obsidian_images/img/20260413110552534.png)

최근 LLM과 결합한 개인지식관리 도구로 더욱 각광받고 있는 옵시디언(Obsidian) 볼트를 활용해서 웹사이트를 개발했습니다. 개발 동기 및 과정, 그리고 본 프로젝트를 활용할 수 있는 방법까지 상세하게 소개하겠습니다.

> [!abstract] 완성된 결과물 미리보기
> 
> 옵시디언의 지식 체계를 웹으로 그대로 연결한 **PROCPA 디지털 가든**의 최종 결과물입니다.
> 
> - **공식 웹사이트**: [procpa.co.kr](https://procpa.co.kr)
> - **GitHub 리포지토리**: [procpalee/procpa_obsidian_style](https://github.com/procpalee/procpa_obsidian_style)

---

## 1. 제작 동기
### 1.1. 옵시디언을 나만의 웹사이트로
평소 **옵시디언(Obsidian)** 을 개인 지식관리 도구로 사용하고 있습니다. 업무에 필요한 자료들 뿐만 아니라 블로그에 작성하는 글들도 모두 마크다운으로 작성하고 있는데, 이 콘텐츠를 그대로 웹페이지로 만들 수 있으면 좋겠다는 생각이 출발점이었습니다.

여기에 옵시디언의 핵심 기능들 — **그래프 뷰(Graph View), 백링크(Backlinks), 위키링크(WikiLinks), 메타데이터(Properties) 기반 탐색 등** — 을 웹에서도 그대로 구현하고 싶었습니다.  또한 옵시디언에서 글을 쓰고 파일만 옮기면 사이트에 바로 반영되는 그런 매끄러운 **워크플로우**를 구축하고 싶었습니다.

### 1.2. 기존 대안의 한계
옵시디언을 활용한 **디지털 가든(Digital Garden)** 을 만드는 방법은 이미 여러 가지가 있고, 대표적으로 아래의 서비스가 있습니다.

- **Obsidian Publish**: 옵시디언 공식 퍼블리시 서비스
- **Quartz**: 옵시디언 마크다운을 정적 사이트로 변환하는 오픈소스

하지만 이들은 디자인이 **획일적**이고 커스터마이징에 한계가 있었습니다. 예를 들어 평소 가이드북 등 시리즈 형태의 긴 호흡을 가진 글들을 자주 작성하다 보니, 이를 체계적으로 보여줄 수 있는 위키독스(WikiDocs) **스타일의 문서(Docs)형 페이지** 구현이 필요했고, 그 외에도 **퍼스널브랜딩**을 위한 다양한 요소를 추가하고 싶었습니다.

또한 **회계/재무 전문성과 AI의 생산성**이라는 아이덴티티를 강조하기 위해서는 남들과 같은 틀에서 벗어나야 한다고 생각했고 원하는 기능도 더 필요했기에 직접 만들기로 결정했습니다.

## 2. 개발 중점사항 및 활용 툴
### 2.1. 인공지능스러움(AI-slop)을 탈피한 디자인 지향
기존에 제작했던 웹사이트에서 가장 아쉬웠던 점은 소위 **'AI스러운(AI-slop)'** 느낌이 강했다는 것이었습니다. 대개 AI 에이전트들은 **글래스모피즘(Glassmorphism)** 이나 화려한 **그라디언트(Gradient)** 색상을 과도하게 제안하는 경향이 있는데, 이는 얼핏 보기에는 화려해 보이지만 실제 상용 서비스로서는 누가 봐도 **'바이브 코딩(Vibe Coding)'** 으로 만들어진 듯한 가벼운 인상을 주기 쉽습니다.
...
### 2.2. 활용 툴 : 클로드 코드(Claude Code)
개발에는 **클로드 코드(Claude Code)**를 프로젝트의 메인 툴로 활용했습니다. 요구사항 및 `Design.md`를 정리한 뒤, **계획(Plan) 모드**를 통해 전체적인 프로젝트의 뼈대를 잡고 아키텍처를 설계했습니다.

특히 개발 과정에서 클로드의 다양한 **스킬(Skills)**들을 적극적으로 활용하였습니다.

- **Frontend 스킬**: Tailwind CSS v4와 shadcn/ui를 활용한 정교한 컴포넌트 설계 및 반응형 레이아웃 구현에 사용되었습니다.
- **Web Search 스킬**: Next.js 15, Velite 등 최신 라이브러리의 변경된 API나 기술 문서를 실시간으로 참조하여 시행착오를 최소화했습니다.
- **File/Code Analysis 스킬**: 대규모 코드베이스 내에서 복잡한 위키링크 파싱 로직이나 그래프 뷰의 성능 최적화 포인트를 정확히 식별하여 해결했습니다.
...
## 6. 웹사이트 구축 방법
나의 옵시디언 볼트(Vault)를 웹사이트로 변환하고 관리하는 전 과정을 **5단계(Phase)** 로 나누어 상세히 설명해 드리겠습니다.

### 6.1. 1단계: 프로젝트 환경 구성
가장 먼저 개발 환경과 콘텐츠를 담을 바구니를 준비해야 합니다.

- **GitHub 저장소 복사(Fork)**: 먼저 [Github 저장소](https://github.com/procpalee/procpa_obsidian_style)에 접속합니다. 우측 상단의 **'Fork'** 버튼을 클릭하여 본인의 계정으로 프로젝트 전체를 복사해 옵니다. 이것이 웹사이트의 핵심 '뼈대'가 됩니다.
...
콘텐츠 생성을 위한 '글쓰기' 단계입니다.

- **카테고리 폴더 만들기**: `content/` 폴더 아래에 `01. 회계실무`, `02. 인사이트`와 같이 번호를 붙인 폴더를 만듭니다. 이 번호는 웹에서의 **정렬 순서**를 결정하며 URL 주소에서는 자동으로 생략됩니다.
- **문서(Markdown) 작성**: 옵시디언에서 평소처럼 글을 작성하되, 문서 최상단에는 아래와 같이 필수 정보를 담은 **프론트매터(Frontmatter)**를 포함해야 합니다.(단, 작성하지 않는 경우에도 title과 description은 자동 추출되고 date는 파일 생성일자로 기록됩니다.)
...
## 마치며
전문가로서의 **퍼스널 브랜딩**을 꿈꾸거나, **개인지식관리 시스템**을 웹사이트로 구축하고 싶은 분들에게 이 프로젝트가 도움이 되기를 바랍니다. 구현 과정에서 어려움을 겪으시거나 궁금한 점이 있다면 언제든 댓글이나 메일로 문의해 주세요.
