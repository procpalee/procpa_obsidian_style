...
- **Tone**: hierarchy from typography, monospace kickers, hairline dividers (`border-border/60`). Korean text gets `word-break: keep-all` (on `<body>`). **Exception — the home hero only**: a duotone image background + gradient overlays + a gradient-shimmer accent word + entrance animations are allowed there (see `src/components/home/hero-fx/*`). Elsewhere keep the no-gradient/no-glow restraint.
...
`src/app/page.tsx`: **Hero → Approach → ServicesTeaser (What I do) → CtaBlock**. Sections (except hero) wrapped in `src/components/reveal.tsx` (IntersectionObserver fade/slide). Hero = `home/hero.tsx` → `home/hero-fx/hero-fx-shell.tsx` (text slide-up + shimmer) + `home/hero-fx/image-duotone.tsx` (background). Home shows teasers that link out to the menu pages — keep it from duplicating full page content.
